...
def api_get_authorizations():
    ...
    auth_requests = (
        ...
        .filter(AuthRequest.active == True).options(  # noqa
            joinedload(AuthRequest.places), joinedload(AuthRequest.motive)
        )
    )
    ...
    if month == "all-months":  # Don't filter by months
        ...
        if year == "last-5-years":  # Filter on a range of 5 years
            ...
            is_emitted = (five_years_ago <= extracted_creation_year) & (
            ...
            is_active = (extracted_auth_start_year <= current_year) & (
            ...
            is_active = (extracted_auth_start_year <= year) & (
        ...
        if year == "last-5-years":  # Filter on a range of 5 years
            ...
            month_start_date = datetime(year, month, 1)
            month_end_date = datetime(year, month, calendar.mdays[month])
            is_active = (AuthRequest.auth_start_date <= month_end_date) & (
                AuthRequest.auth_end_date >= month_start_date
            )
            if status == "active":
                auth_requests = auth_requests.filter(is_active)
    ...
    auth_requests = auth_requests.order_by(AuthRequest.number.desc()).all()
    ...
    return jsonify({"canEdit": g.user.id_droit_max > 1, "authRequests": auth_requests})
